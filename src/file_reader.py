import pandas as pd


def csv_file_reader(path_to_file: str = 'D:/Pyton/package-jura_neurolink/data/transactions.csv') -> list:
    """Функция для считывания финансовых операций из CSV, принимает путь к файлу CSV в качестве аргумента.
    Возвращает список словарей с транзакциями."""

    try:
        csv_df = pd.read_csv(path_to_file, delimiter=';').fillna(False)
        return csv_df.to_dict('records')
    except Exception:
        return []


def excel_file_reader(path_to_file: str = 'D:/Pyton/package-jura_neurolink/data/transactions_excel.xlsx') -> list:
    """Функция для считывания финансовых операций из Excel, принимает путь к файлу Excel в качестве аргумента.
    Возвращает список словарей с транзакциями."""

    try:
        excel_df = pd.read_excel(path_to_file).fillna(False)
        return excel_df.to_dict('records')
    except Exception:
        return []
