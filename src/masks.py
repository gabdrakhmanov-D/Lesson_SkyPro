...
def get_mask_card_number(card_number: str) -> str:
    ...
    if not card_number.isdigit() or len(card_number) != 16:
        logger.error('Ошибка: неверный формат номера карты')
        raise ValueError('Номер карты должен состоять из 16 цифр!')
    logger.info('Возврат маски номера карты')
...
def get_mask_account(account_number: str) -> str:
    ...
    if not account_number.isdigit() or len(account_number) != 20:
        logger.error('Ошибка: неверный формат номера счета')
        raise ValueError("Номер счета должен состоять из 20 цифр!")
    logger.info('Возврат маски счета')
